...
class MonkeyTools:
    def __init__(self, monkey_list):
        ...
        self.modulo_product = np.prod([m.test for m in monkey_list])
...
class Monkey:
    ...
    def do_turn(self, tools):
        for item in self.items:
            self.inspections += 1
            item_worry = self.do_operation(item, tools)
